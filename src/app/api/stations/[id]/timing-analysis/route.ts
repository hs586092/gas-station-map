...
export async function GET(
  ...
  if (!oilRaw || oilRaw.length < 7) {
    return NextResponse.json({
      oilEvents: [], competitorSpeed: [], timingImpact: null,
      currentSituation: { pendingReaction: false, message: "유가 데이터가 부족합니다.", urgency: "none" as const },
      dataStatus: { totalEvents: 0, minRequired: 3, isReliable: false, dataRange: { from: "", to: "" } },
    });
  }
