      ...
      <main className="px-5 pb-10">
        ...
        {reflection && (
          <div className={`mb-6 rounded-xl p-5 border border-border ${
            ...
            {reflection.priceChange !== null && (
              <p className="text-[12px] text-slate-800 m-0 mt-1">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-6">
          <div className="bg-surface-raised rounded-xl p-4 border border-border">
            <div className="text-[13px] text-slate-800 mb-1">감지된 유가 이벤트</div>
            <div className="text-[22px] font-extrabold text-slate-900">{events.length}<span className="text-[12px] font-normal">건</span></div>
          </div>
          <div className="bg-surface-raised rounded-xl p-4 border border-border">
            <div className="text-[13px] text-slate-800 mb-1">평균 반영 속도</div>
            <div className="text-[22px] font-extrabold text-slate-900">
              {avgReflectDays != null ? <>{avgReflectDays}<span className="text-[12px] font-normal">일</span></> : <span className="text-[14px] text-slate-700">데이터 부족</span>}
            </div>
          </div>
          <div className="bg-surface-raised rounded-xl p-4 border border-border">
            <div className="text-[13px] text-slate-800 mb-1">반영 완료율</div>
        ...
        {oilStory && (
          <div className="bg-surface-raised rounded-xl p-5 border border-border mb-6">
            ...
            <div className="text-[12px] text-slate-800 leading-relaxed">{oilStory}</div>
          </div>
        )}
        ...
        <div className="bg-surface-raised rounded-xl p-5 border border-border mb-6">
          ...
          <div className="text-[13px] text-slate-800 mb-4">Brent 유가와 2주 후 내 소매가를 같은 시점에 겹쳐 표시</div>
        ...
        <div className="bg-surface-raised rounded-xl p-5 border border-border">
          ...
          <div className="text-[13px] text-slate-800 mb-4">Brent 주간 변동 ±$3 이상 감지</div>

          {events.length === 0 ? (
            <div className="text-[13px] text-slate-700 text-center py-8">
                    ...
                    <div className={`rounded-xl p-4 ${ev.direction === "up" ? "bg-red-50" : "bg-blue-50"}`}>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className={`text-[12px] px-2 py-0.5 rounded-full font-bold ${
                          ev.reflected ? "bg-emerald-900/50 text-emerald-700" : "bg-slate-100 text-slate-700"
                        }`}>
                      ...
                      <div className="text-[13px] text-slate-800">
                        ...
                        <span className="text-slate-700"> ({ev.startDate.slice(5)} ~ {ev.endDate.slice(5)})</span>
                      </div>
                      {ev.retailChange != null && (
                        <div className="text-[13px] text-slate-800 mt-1">
                          ...
                          {ev.daysToReflect != null && (
                            <span className="text-slate-700"> (반영까지 {ev.daysToReflect}일)</span>
                          )}
                        </div>
                      )}
                    </div>
