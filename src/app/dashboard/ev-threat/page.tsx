              ...
              <table className="w-full text-[11px]">
                <thead>
                  <tr className="border-b border-border">
                    ...
                    <th className="text-left py-2 px-2 text-text-secondary font-semibold">운영사</th>
                    <th className="text-right py-2 px-2 text-text-secondary font-semibold">급속</th>
                    <th className="text-right py-2 px-2 text-text-secondary font-semibold">완속</th>
                    <th className="text-right py-2 px-2 text-text-secondary font-semibold">거리</th>
                  </tr>
                </thead>
                <tbody>
                  {chargers.map((c) => (
                    <tr key={c.station_id} className="border-b border-border/50 hover:bg-surface transition-colors">
                      <td className="py-2 px-2 text-text-primary font-medium">{c.station_name}</td>
                      <td className="py-2 px-2 text-text-secondary">{c.operator}</td>
                      <td className="py-2 px-2 text-right font-bold text-coral">{c.fast_count > 0 ? c.fast_count : "-"}</td>
                      <td className="py-2 px-2 text-right text-text-secondary">{c.slow_count > 0 ? c.slow_count : "-"}</td>
                      <td className="py-2 px-2 text-right text-text-secondary">{c.distance_km}km</td>
                    </tr>
                  ))}
                </tbody>
              </table>
