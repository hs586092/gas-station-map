 ...
 */
export async function getCarwashSummary(
  id: string,
  options?: { compact?: boolean; weatherForecast?: any | null }
): Promise<any> {
  const compact = options?.compact ?? false;
  const wxForecastRes = options?.weatherForecast ?? null;

  // ── 1. carwash_daily + weather_daily + forecast_history + sales_data 병렬 조회 ──
  const [cwRes, weatherRes, fcRes, salesCountRes] = await Promise.all([
    supabase
      .from("carwash_daily")
